...
function validateChileRUT(rut: string): string {
  ...
  const expected = expectedDV === '11' ? '0' : expectedDV === '10' ? 'K' : expectedDV;
...
export const rutSchema = z
  ...
  .refine(val => /^(\d{7,8})-([0-9K])$/i.test(val), 'RUT debe tener formato: 12345678-K')
...
export const phoneSchema = z
  ...
  .regex(
    /^(\+56|0)?9\d{8}$/,
    'Teléfono debe ser: +56912345678 o 912345678'
  )
  .transform(val => {
...
export const cartItemSchema = z.object({
  ...
  quality: z
    .string()
    .refine(val => ['PK', 'G5', 'G4', 'G3'].includes(val), {
      message: 'Calidad debe ser: PK, G5, G4 o G3'
    }),

  size: z
    .string()
    .min(1, 'Talla requerida')
    .max(10),
  ...
  image: z
    .string()
    .url('Imagen debe ser URL válida')
    .max(500)
    .optional(),
});
